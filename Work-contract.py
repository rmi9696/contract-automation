...
document_path = Path(__file__).parent / "work-contract.docx"
doc = DocxTemplate(document_path)

today = datetime.datetime.today()
today_in_one_week = today + datetime.timedelta(days=7)

# GUI with six input fields and two buttons
layout = [
    [sg.Text("Client Name:"), sg.Input(key="CLIENT", do_not_clear=False)],
    [sg.Text("Idividual Name:"), sg.Input(key="VENDOR", do_not_clear=False)],
    [sg.Text("Idividual Email:"), sg.Input(key="VENDOR", do_not_clear=False)],
...
while True:
    ...
    if event == "Create Contract":
        # Add calculated fields to our dictionary 
        
        values["NONREFUNDABLE"] = round(float(values["AMOUNT"]) * 0.014, 2)
